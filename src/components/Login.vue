    ...
    <div class="login" style="margin-top:100px;margin-left: 65%;  " v-if="isshowing">
      <div class="header">
        <div class="switch" id="switch" >
          ...
          <a class="switch_btn"  id="switch_login"  tabindex="8" @click="changetag(1)">快速注册</a>
          <div class="switch_bottom" id="switch_bottom" style="position: absolute; width: 64px;margin-left: 0%"></div>
        </div>
      </div>
      ...
      <Form v-show="tagdisplay.flag2" ref="registerformCustom" :model="registerformCustom" :rules="ruleCustom" :label-width="80">
        ...
        <FormItem label="身份：" >
          <RadioGroup v-model="registerformCustom.registerIdentity" @on-change="changebutton" style="float: left">
    ...
    methods: {
      ...
      changetag(num){
        if (num==0){
          ...
          document.getElementById("switch_bottom").style.marginLeft="0%"

        }
        else {
          this.tagdisplay.flag1=false
          this.tagdisplay.flag2=true
          document.getElementById("switch_bottom").style.marginLeft="70%"
        }
      },
      handleSubmit (name) {
